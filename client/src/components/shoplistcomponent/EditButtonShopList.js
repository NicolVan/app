...
const EditButtonShopList = ({ listId, item, onEdit }) => {
  ...
  const handleEditSubmit = async () => {
    try {
      const response = await axios.put(`${API_URL}/shoppingList/${listId}/item/${item._id}`, editItem, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
      onEdit(listId, response.data);
      closeModal();
    } catch (error) {
